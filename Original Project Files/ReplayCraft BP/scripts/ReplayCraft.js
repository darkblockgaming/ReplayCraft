...
function saveFrameDataRC(player) {
...
function retrieveFrameDataRC(player) {
	...
	const retrievedPData = pDataString ? JSON.parse(pDataString) : null;
	const retrievedRData = rDataString ? JSON.parse(rDataString) : null;
...
function saveSettDataRC(player) {
...
function retrieveSettDataRC(player) {
	const cData = cineCDataMap.get(player.id);
	if (cData.retrievedSett === true) return;
	const eDataString = world.getDynamicProperty(`eData_${player.id}`);
	const retrievedEData = eDataString ? JSON.parse(eDataString) : null;
...
function ReplayCraft(player) { //if replayStateMachine.state = recPending 
...
function cineSettings(player) {
	...
	replaySettingsForm.show(player).then(response => {
		...
		eData.easingTime = (response.formValues[1] == null || response.formValues[1] <= 0) ? 1 : Math.floor(response.formValues[1]);
...
function removeLastFrame(player) {
	const pData = cinePDataMap.get(player.id);
	const rData = cineRDataMap.get(player.id);
	const eData = cineEDataMap.get(player.id);
	const cData = cineCDataMap.get(player.id);
...
function removeAllFrames(player) {
	const pData = cinePDataMap.get(player.id);
	const rData = cineRDataMap.get(player.id);
	const eData = cineEDataMap.get(player.id);
	const cData = cineCDataMap.get(player.id);
...
function showParticle() {
	system.runInterval(() => {
		for (const player of world.getPlayers()) {
			...
			pData.cineCamPos.map(pos => player.runCommand(`particle ${partSelected} ${pos.x} ${pos.y} ${pos.z}`));
		}
	});
}
