...
import { cameraPlaybackMenu } from "../path-placement/camera-playback-menu";
...
import { frameManagementMenu } from "../path-placement/manage-frames";
...
export function panoramicCinematic(player: Player) {
    ...
    const replayForm = new ActionFormData().title("rc2.title.cinematic.menu").body("rc2.body.create.panoramic.cine").button("rc2.button.add.focus.point");
    ...
    replayForm.show(player).then((result) => {
        ...
        actions[index++] = () => addCameraFrame(player, true); //add a focus point instead if camera frames
